...
> 状态：**已实施（2026-08-15）**。v1.0.2 代码已落地并通过端到端冒烟测试（见 `test_notify_smoke.py`）。
...
| 3 | 桌宠 MCP 端口只提 18791 | Nanobot 网关 health 实为 `127.0.0.1:18790`（`config.json` menus[].Nanobot.endpoint），WebUI 8765。桌宠 MCP 用 **18791** 以示区分 | 端口避免混淆 |
...
- `_build_tray_menu` 的 `more_items`：加"🔔 通知设置"项（复用 `ConfigViewer`）。
...
- 绑定：`127.0.0.1` 仅本机（安全），默认端口 **18791**（与 Nanobot 网关 18790、WebUI 8765 区分）。
...
- **用户可控**：托盘菜单"🔔 通知设置"可暂停/按来源开关；配置写入 `desktop_aipet.notifications`。
...
> 注：`endpoints`(18790) 是 Nanobot 网关 health；桌宠 MCP `port`(18791) 是供 Nanobot/WorkBuddy **反向调用**的端口，二者区分。来源启用开关 `notify` 直接挂在已有菜单块上，不另起 `sources`。
...
URL = "http://127.0.0.1:18791/mcp"; TOKEN = "<token>"
...
| M4 | 托盘"🔔 通知设置"菜单 + `ConfigViewer` 复用 | 菜单可见、可暂停 |
