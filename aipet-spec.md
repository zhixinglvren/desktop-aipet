...
- 托盘/桌宠菜单提供对 Gateway 的启停、WebUI、工作空间跳转、渠道授权、配置与日志查看。
...
- **顶层菜单标题**: 固定显示 "🟢/🟡/🔴 桌面AI助理", 使用**聚合状态**。
- **各监控项子菜单标题**: 显示 "🟢 MonitorName" 或 "🔴 MonitorName", **仅二态** (正常=绿, 异常=红), 不显示 "HTTP 200" 等探测详情。
- **鼠标悬停提示 (tooltip)**: 仅保留 "桌面AI助理", 不带 emoji / 状态摘要 / 圆点。

**文字规则**: 菜单标题只写 "桌面AI助理" 或监控名, **不写** "全部正常"/"异常"/"HTTP 200" 等状态摘要文字。

**派生规则**: 由所有启用的 `monitor` 健康结果聚合 — 全正常→`normal`/`🟢`; 有正常有异常→`warning`/`🟡`; 全异常→`error`/`🔴`。`warning` 仅出现在聚合层, 单个 monitor 没有黄色状态。
...
每个菜单动作是一个 JSON 对象, **必须显式声明 `type`**。旧配置用 `url`/`file`/`cmd` 作为键名仍可兼容 (`_action_type` 会回退识别), 但新配置一律用 `type` 字段。
...
| `open_workspace` | 读取配置中的 workspace 字段并打开文件夹 | `config_path`, `config_key` (或 monitor 级同名字段) |
| `popup_file` | 弹出只读文件查看器 (JSON 自动格式化) | `path`, `title`, `pretty_json` |
| `popup_log` | 弹出实时尾随日志窗口 | `path` (或 monitor 级 `log_path`), `refresh_ms`, `tail_lines`, `title` |
...
- `monitor.actions[]` 的顺序 **即菜单显示顺序**, 不要依赖任何隐式排序。
- `monitor.config_path` / `log_path` / `workspace_key` 为动作提供默认值 (动作内未指定时用 monitor 级值)。
...
- 所有写入磁盘的文本 (日志、配置导出) 必须 **UTF-8 无 BOM**。
...
1. ❌ 在托盘回调里直接 `subprocess.Popen` / `os.startfile` / 建 `Toplevel` — 必须 `post_ui()`。
2. ❌ 把动作逻辑硬编码进 `_execute_action` 的 `if/elif` 分支 — 必须走 `type` 配置 + `_do_<type>`。
3. ❌ 依赖 stdout/stderr 调试 `pythonw` 程序。
4. ❌ 用 `Tee-Object` 写日志文件 (编码问题)。
5. ❌ 每轮健康检测都 `update_menu()` — 仅状态变化时重建。
6. ❌ 硬编码 `%USERPROFILE%` 之外的绝对用户路径 — 用 `expand()` + 配置变量。
...
`desktop_aipet` 字段说明:
`pet_theme`(主题, 当前 `robot`), `pet_visible`(是否显示桌宠), `pet_scale`(缩放, 默认 1.0),
`pet_x`/`pet_y`(桌宠坐标, 运行时自动保存), `check_interval_s`(健康检测间隔, 默认 30),
`log_retention_days`(日志保留天数), `auto_restart_on_failure`(失败自启开关)。

---

## 11. Nanobot 托盘菜单 (规范样例, 即 config.json 实际内容)

顺序与行为须严格如下:

1. **启动 Gateway** (`script`) — 隐藏窗口执行 `start-gateway.bat`, 同时 tee 输出到 `gateway.log`。
2. **重启 Gateway** (`script_seq`) — 先 `stop-gateway.bat` (隐藏、等待完成), 间隔 3 秒后执行 `start-gateway.bat` (隐藏 + tee 日志)。
3. **打开 WebUI** (`url`) — 打开 `http://127.0.0.1:8765`。
4. **跳转工作空间** (`open_workspace`) — 读取 nanobot 配置 `agents.defaults.workspace` 并打开该资源文件夹。
...
完整 `config.json` 见仓库内 `config.json` (与本节一致)。
...
- [ ] 状态 emoji 是否仅出现在菜单标题与 Nanobot 子菜单项, 机器人图标/桌宠无状态徽标?
